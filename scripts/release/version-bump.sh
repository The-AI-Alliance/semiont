...
echo ""
echo "Next: open a PR for this branch and merge it, then release from main:"
echo ""
echo "  ./scripts/release/preflight.sh                  # is main releasable?"
echo "  gh workflow run release.yml                     # publishes every channel"
echo "  ./scripts/release/notes-context.sh              # assemble the post's context"
echo "  ./scripts/release/announce.sh ${NEXT} <draft.md> --post"
echo "  ./scripts/release/verify-release.sh ${NEXT}"
