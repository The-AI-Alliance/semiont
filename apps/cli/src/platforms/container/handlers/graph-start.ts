...
async function startJanusGraph(context: ContainerStartHandlerContext): Promise<StartHandlerResult> {
  ...
  try {
    ...
    if (output.includes('semiont-janusgraph')) {
      if (!service.quiet) {
  ...
  try {
    ...
    for (let i = 0; i < maxAttempts; i++) {
      ...
      try {
        // Try to execute a simple Gremlin query
        execSync('docker exec semiont-janusgraph bin/gremlin.sh -e "g.V().count()"', {
          stdio: 'ignore',
          timeout: 5000
        });
        ready = true;
        ...
        // Not ready yet
        if (!service.quiet && i % 5 === 0) {
          printInfo(`Still waiting... (${i}/${maxAttempts})`);
        }
      }
    }
    ...
    if (!service.quiet) {
      ...
      printInfo('  docker exec -it semiont-janusgraph bin/gremlin.sh');
    }
