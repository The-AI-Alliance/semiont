...
const provisionDatabaseContainer = async (context: ContainerProvisionHandlerContext): Promise<ProvisionHandlerResult> => {
  ...
  if ((hasInitScripts || hasMigrations || hasSeedData) && !containerRunning) {
    ...
    try {
      execFileSync(runtime, runArgs, { encoding: 'utf-8' });
      ...
      while (attempts < maxAttempts) {
        try {
          if (image.includes('postgres')) {
            execFileSync(runtime, ['exec', initContainerName, 'pg_isready', '-U', dbUser || 'postgres'], { stdio: 'ignore' });
            break;
          } else if (image.includes('mysql')) {
            execFileSync(runtime, ['exec', initContainerName, 'mysqladmin', 'ping', '-h', 'localhost'], { stdio: 'ignore' });
            break;
          }
        } catch {
      ...
      execFileSync(runtime, ['stop', initContainerName], { stdio: 'ignore' });
      execFileSync(runtime, ['rm', initContainerName], { stdio: 'ignore' });
