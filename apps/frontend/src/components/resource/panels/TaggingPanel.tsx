...
export function TaggingPanel({
  ...
  const { sortedAnnotations: sortedTags, containerRef, handleAnnotationRef } =
    useAnnotationPanel(tags, hoveredTagId);
  ...
  const handleDetect = () => {
    if (onDetectTags && selectedCategories.size > 0) {
      onDetectTags(selectedSchemaId, Array.from(selectedCategories));
  ...
  return (
    <div className="flex flex-col h-full bg-white dark:bg-gray-900">
      <PanelHeader annotationType="tag" count={tags.length} title={t('title')} />
