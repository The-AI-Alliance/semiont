/**
 * Detect Highlights Stream SSE Endpoint Tests
 *
 * Tests the highlight detection endpoint with density parameter support.
 */
...
describe('POST /resources/:id/detect-highlights-stream', () => {
  ...
  beforeAll(async () => {
    ...
    testDir = join(tmpdir(), `semiont-test-highlights-${Date.now()}`);
