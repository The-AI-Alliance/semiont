/**
 * Detect Assessments Stream SSE Endpoint Tests
 *
 * Tests the assessment detection endpoint with tone and density parameter support.
 */
...
describe('POST /resources/:id/detect-assessments-stream', () => {
  ...
  beforeAll(async () => {
    ...
    testDir = join(tmpdir(), `semiont-test-assessments-${Date.now()}`);
