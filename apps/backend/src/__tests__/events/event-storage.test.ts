...
describe('EventStorage', () => {
  ...
  describe('Sequence Tracking', () => {
    ...
    it('should restore sequence number from existing events', async () => {
      ...
      await newStorage.initializeResourceStream(resourceId('doc1'));
      expect(newStorage.getSequenceNumber(resourceId('doc1'))).toBe(2);
  ...
  describe('Sharding', () => {
    it('should calculate shard path for resource IDs', () => {
      const path1 = storage.getShardPath(resourceId('doc-abc123'));
      expect(path1).toMatch(/^[0-9a-f]{2}\/[0-9a-f]{2}$/);
    });

    it('should create consistent shard paths for same resource', () => {
      const path1 = storage.getShardPath(resourceId('doc-test123'));
      const path2 = storage.getShardPath(resourceId('doc-test123'));
      expect(path1).toBe(path2);
    });

    it('should bypass sharding for __system__ events', () => {
      const path = storage.getShardPath(resourceId('__system__'));
      expect(path).toBe('');
    });
  ...
  describe('File Rotation', () => {
    ...
    it('should read events from multiple files', async () => {
      ...
      const allEvents = await storage.getAllEvents(resourceId('doc1'));
  ...
  describe('Read Operations', () => {
    ...
    it('should return empty array for nonexistent resource', async () => {
      const events = await storage.getAllEvents(resourceId('doc-nonexistent'));
    ...
    it('should return null for last event of empty file', async () => {
      // Create empty file
      const docPath = storage.getResourcePath(resourceId('doc-empty'));
  ...
  describe('JSONL Format', () => {
    it('should write valid JSONL (one object per line)', async () => {
      ...
      const docPath = storage.getResourcePath(resourceId('doc1'));
