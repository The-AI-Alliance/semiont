...
describe('EventSubscriptions', () => {
  ...
  describe('Resource Subscriptions', () => {
    it('should subscribe to resource events', () => {
      ...
      const sub = subscriptions.subscribe(resourceUri('doc1'), callback);

      expect(sub.resourceUri).toBe('doc1');
    ...
    it('should notify resource subscribers', async () => {
      const callback = vi.fn();
      subscriptions.subscribe(resourceUri('doc1'), callback);

      const event = createStoredEvent('annotation.added', 'doc1');
      await subscriptions.notifySubscribers(resourceUri('doc1'), event);
    ...
    it('should support multiple subscribers for same resource', async () => {
      ...
      subscriptions.subscribe(resourceUri('doc1'), callback1);
      subscriptions.subscribe(resourceUri('doc1'), callback2);
      subscriptions.subscribe(resourceUri('doc1'), callback3);
    ...
    it('should not notify subscribers of other resources', async () => {
      ...
      subscriptions.subscribe(resourceUri('doc1'), callback1);
      subscriptions.subscribe(resourceUri('doc2'), callback2);
    ...
    it('should clean up empty subscription sets', () => {
      const sub = subscriptions.subscribe(resourceUri('doc1'), vi.fn());

      expect(subscriptions.getSubscriptionCount(resourceUri('doc1'))).toBe(1);

      sub.unsubscribe();

      expect(subscriptions.getSubscriptionCount(resourceUri('doc1'))).toBe(0);
    });

    it('should get subscription count for resource', () => {
      subscriptions.subscribe(resourceUri('doc1'), vi.fn());
      subscriptions.subscribe(resourceUri('doc1'), vi.fn());
      subscriptions.subscribe(resourceUri('doc2'), vi.fn());

      expect(subscriptions.getSubscriptionCount(resourceUri('doc1'))).toBe(2);
      expect(subscriptions.getSubscriptionCount(resourceUri('doc2'))).toBe(1);
      expect(subscriptions.getSubscriptionCount(resourceUri('doc-nonexistent'))).toBe(0);
    });
  ...
  describe('Global Subscriptions', () => {
    ...
    it('should not affect resource subscriptions', async () => {
      ...
      subscriptions.subscribe(resourceUri('doc1'), docCallback);
      ...
      await subscriptions.notifySubscribers(resourceUri('doc1'), docEvent);
  ...
  describe('Fire-and-Forget Pattern', () => {
    it('should not block on slow subscribers', async () => {
      ...
      subscriptions.subscribe(resourceUri('doc1'), fastCallback);
      subscriptions.subscribe(resourceUri('doc1'), slowCallback);
    ...
    it('should handle errors in async callbacks without affecting others', async () => {
      ...
      subscriptions.subscribe(resourceUri('doc1'), goodCallback);
      subscriptions.subscribe(resourceUri('doc1'), badCallback);
      subscriptions.subscribe(resourceUri('doc1'), anotherGoodCallback);
    ...
    it('should support async callbacks', async () => {
      ...
      subscriptions.subscribe(resourceUri('doc1'), asyncCallback);
  ...
  describe('Edge Cases', () => {
    it('should handle notification with no subscribers', async () => {
      ...
      await expect(subscriptions.notifySubscribers(resourceUri('doc-nonexistent'), event)).resolves.toBeUndefined();
    });
    ...
    it('should allow re-subscribing after unsubscribing', async () => {
      ...
      const sub1 = subscriptions.subscribe(resourceUri('doc1'), callback);
      sub1.unsubscribe();

      const sub2 = subscriptions.subscribe(resourceUri('doc1'), callback);
