...
// Shared router type
export type AnnotationsRouterType = Hono<{ Variables: { user: User; config: EnvironmentConfig; eventBus: EventBus; makeMeaning: Awaited<ReturnType<typeof startMakeMeaning>> } }>;

// Create a router with auth middleware pre-applied
export function createAnnotationRouter(): AnnotationsRouterType {
  const router = new Hono<{ Variables: { user: User; config: EnvironmentConfig; eventBus: EventBus; makeMeaning: Awaited<ReturnType<typeof startMakeMeaning>> } }>();
