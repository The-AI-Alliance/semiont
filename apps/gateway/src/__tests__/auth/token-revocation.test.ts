/**
 * Per-user revocation epoch for gateway-minted tokens: every token carries
 * the user's `tokenVersion` at mint; logout bumps `User.tokenVersion`, so a
 * token whose `tokenVersion` is behind the user's current value is rejected.
 * Prisma is mocked (same harness as auth-integration), so the tests simulate
 * the post-logout state by returning a bumped user from findUnique.
 */
