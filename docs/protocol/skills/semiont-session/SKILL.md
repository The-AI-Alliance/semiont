...
You are helping a user build a long-running Semiont script — a watcher, a daemon, anything that runs longer than a single access token's life (short — see docs/system/administration/AUTHENTICATION.md; it is minutes, not hours). The lighter `SemiontClient` setup the other skills use is fine for one-shot scripts; for anything that needs to keep working after a token expires, `SemiontSession` from `@semiont/sdk` owns the refresh, validation, and storage machinery for you.
...
- **Errors split by surface.** Per-call rejections come from namespace methods and extend `SemiontError` — narrow to `APIError` (HTTP, with `status`) or `BusRequestError` (bus-mediated, with codes like `bus.timeout`). Asynchronous session-fatal errors (`session.auth-failed`, `session.refresh-exhausted`, `browser.sign-in-failed`) arrive on `SemiontBrowser.error$` as `SemiontSessionError` and surface through `onError`; subscribe to that observable in long-running scripts so you don't miss them. See [Error Handling in Usage.md](../../../../packages/sdk/docs/Usage.md#error-handling).
