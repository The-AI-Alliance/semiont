...
- [Attend Flow](./ATTEND.md) - Hover/focus/sparkle coordination (attention precedes navigation)
...
|-------|---------|-------------|
| `navigation:click` | `{ annotationId: string; motivation: Motivation }` | User clicked an annotation element |
...
Subscribers:
- **useAttentionFlow** — relays as `attend:focus` to scroll the document view to the annotation
...
| `navigation:panel-open` | `{ panel: string; scrollToAnnotationId?: string; motivation?: string }` | Open a specific panel, optionally scrolling to an annotation |
| `navigation:panel-close` | `void` | Close the active panel |

**`navigation:panel-toggle`** is emitted by the Toolbar when a user clicks a panel button. If the panel is already open, it closes; otherwise it opens.

**`navigation:panel-open`** is emitted when the system needs to open a specific panel programmatically — for example, when an annotation is clicked in the document view (ResourceViewer opens the annotations panel with a scroll target) or when an annotation is requested (useAnnotationFlow opens the relevant motivation tab).

**`navigation:panel-close`** closes whichever panel is currently active.
...
| Event | Payload | Description |
|-------|---------|-------------|
| `navigation:sidebar-toggle` | `void` | Toggle the left sidebar collapsed/expanded |
| `navigation:resource-close` | `{ resourceId: string }` | Close a resource tab in the sidebar |
| `navigation:resource-reorder` | `{ oldIndex: number; newIndex: number }` | Reorder resource tabs (keyboard or drag-and-drop) |
...
| Event | Payload | Description |
|-------|---------|-------------|
| `navigation:router-push` | `{ path: string; reason?: string }` | Navigate to an in-app route |
| `navigation:external-navigate` | `{ url: string; resourceId?: string; cancelFallback: () => void }` | Navigate to an external URL |
| `navigation:reference-navigate` | `{ documentId: string }` | Navigate to a resolved reference's target resource |
| `navigation:link-clicked` | `{ href: string; label?: string }` | A tracked link was clicked (observability) |
| `navigation:entity-type-clicked` | `{ entityType: string }` | User clicked an entity type to filter by it |

**`navigation:router-push`** is emitted by **useObservableRouter** (a wrapper around Next.js `router.push` / `router.replace`) and by ResourceViewerPage for actions like clone-resource and entity-type filtering. Consumed by **NavigationHandler** in the frontend, which performs the actual `router.push()`.
...
**`navigation:reference-navigate`** is emitted by **CodeMirrorRenderer** when a user clicks a resolved reference widget in the document. Consumed by **ResourceViewerPage**, which navigates to the target document.

**`navigation:link-clicked`** is emitted by **ObservableLink** for analytics and observability. No active subscribers — the event exists for future logging or telemetry consumers.

**`navigation:entity-type-clicked`** is emitted when a user clicks an entity type tag to filter resources by that type. Consumed by **ResourceViewerPage**, which applies the entity type filter.
