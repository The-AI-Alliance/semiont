...
| **[CONTAINER-TOPOLOGY.md](CONTAINER-TOPOLOGY.md)** | Multi-container deployment: how the four Semiont-code containers (browser, backend, worker, smelter) and four infrastructure containers (postgres, neo4j, qdrant, ollama) fit together; the unified bus contract and `SemiontSession`; deployment platforms (POSIX / Container / AWS). *Diagram: container topology.* |
