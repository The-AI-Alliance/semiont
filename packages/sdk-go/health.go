...
// Health is what a caller needs to know whether the gateway is serving.
type Health interface {
	// HealthCheck asks the gateway to report itself.
	//
	...
	// calling that "down" would take a gateway offline over a field rename.
...
// NewHealthClient builds a Health against a gateway ORIGIN — "http://host:port",
...
func (h *HealthClient) HealthCheck(ctx context.Context) (*HealthResponse, error) {
	...
	if resp.StatusCode < 200 || resp.StatusCode > 299 {
		return nil, fmt.Errorf("gateway health: %s", resp.Status)
	}
