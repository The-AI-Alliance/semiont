 ...
 *
 * Tests that CommentAnnotationWorker emits proper job progress events to Event Store
 * during comment detection processing.
 */

import { describe, it, expect, beforeAll, afterAll, vi } from 'vitest';
import { CommentAnnotationWorker } from '../../../workers/comment-annotation-worker';
import { JobQueue, type CommentDetectionJob, type RunningJob, type CommentDetectionParams, type CommentDetectionProgress, type ContentFetcher } from '@semiont/jobs';
...
describe('CommentAnnotationWorker - Event Emission', () => {
  ...
  beforeAll(async () => {
    ...
    worker = new CommentAnnotationWorker(jobQueue, config, testEventStore, mockInferenceClient.client, new EventBus(), mockContentFetcher, mockLogger);
