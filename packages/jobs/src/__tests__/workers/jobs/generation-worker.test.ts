...
import { FsJobQueue as JobQueue, type RunningJob, type GenerationParams, type YieldProgress } from '@semiont/jobs';
...
describe('GenerationWorker - Event Emission', () => {
  ...
  beforeAll(async () => {
    ...
    testDir = join(tmpdir(), `semiont-test-generation-worker-${uuidv4()}`);
