...
Workers never persist directly — the **Stower** actor subscribes to the emitted commands and handles all persistence (`eventStore.appendEvent()`). In the gateway deployment the Stower runs inside the **Archivist** service (`archivist-main`), not the backend. Both make-meaning composition roots — [`startMakeMeaning()`](../src/service.ts) (standalone/scripting, all actors in-process) and [`startMakeMeaningGateway()`](../src/service.ts) (the backend, which hosts no actors) — own a `JobQueue` and register the bus command handlers; neither instantiates workers.
