...
async function project(): Promise<SemiontProject> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), 'jq-select-'));
  return new SemiontProject(dir, { anchoredTextDir: `${dir}/anchored-text` });
}
...
describe('jobQueueFor — driver selection (JOB-QUEUE-DRIVER P2)', () => {
  test("an absent [services.jobs] section selects the fs driver (today's behavior, until P3)", async () => {
    const q = jobQueueFor(undefined, await project(), mockLogger, new EventBus());
    expect(q).toBeInstanceOf(FsJobQueue);
  });

  test("type 'fs' selects the fs driver explicitly", async () => {
    const q = jobQueueFor({ type: 'fs' }, await project(), mockLogger, new EventBus());
    expect(q).toBeInstanceOf(FsJobQueue);
  });

  test("type 'jetstream' selects the JetStream driver with its address", async () => {
    const q = jobQueueFor({ type: 'jetstream', servers: '127.0.0.1:4222' }, await project(), mockLogger, new EventBus());
    expect(q).toBeInstanceOf(JetStreamJobQueue);
  });

  test("type 'jetstream' without servers refuses loudly, naming the field", async () => {
    expect(() => jobQueueFor({ type: 'jetstream' }, undefined as never, mockLogger, new EventBus()))
  ...
  test('a ${VAR} placeholder in servers resolves from the environment', async () => {
    ...
    try {
      const q = jobQueueFor({ type: 'jetstream', servers: '${JQ_SELECT_TEST_HOST}:4222' }, await project(), mockLogger, new EventBus());
      expect(q).toBeInstanceOf(JetStreamJobQueue);
    } finally {
  ...
  test('a placeholder naming an UNSET variable refuses loudly, naming the variable', async () => {
    expect(() => jobQueueFor({ type: 'jetstream', servers: '${JQ_SELECT_UNSET_VAR}:4222' }, undefined as never, mockLogger, new EventBus()))
