...
describe('EventBus scoping', () => {
  ...
  it('scopes events by resource', () => {
    ...
    resource1.get('annotate:detect-progress').subscribe(e => events1.push(e));
    resource2.get('annotate:detect-progress').subscribe(e => events2.push(e));

    resource1.get('annotate:detect-progress').next({ status: 'started' });
    resource2.get('annotate:detect-progress').next({ status: 'complete' });
  ...
  it('allows nested scoping', () => {
    ...
    resourceScope.get('annotate:detect-progress').subscribe(e => resourceEvents.push(e));
    subsystemScope.get('annotate:detect-progress').subscribe(e => subsystemEvents.push(e));

    // Events to different scopes are isolated
    resourceScope.get('annotate:detect-progress').next({ status: 'started', message: 'resource level' });
    subsystemScope.get('annotate:detect-progress').next({ status: 'started', message: 'subsystem level' });
  ...
  it('maintains type safety across scopes', () => {
    ...
    const subject = resourceScope.get('annotate:detect-progress');
