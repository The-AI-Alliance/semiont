...
describe('withDeadline', () => {
  ...
  it('passes a value through untouched when the dependency answers', async () => {
    await expect(withDeadline('Graph database', DEADLINE_MS, () => Promise.resolve('ok'))).resolves.toBe('ok');
  });

  it('propagates the original error rather than masking it', async () => {
    const boom = new Error('ECONNREFUSED 127.0.0.1:7687');
    await expect(withDeadline('Graph database', DEADLINE_MS, () => Promise.reject(boom))).rejects.toThrow(
      'ECONNREFUSED',
    );
  });
  ...
  it('rejects — naming the dependency — when the connect never settles', async () => {
    ...
    const raced = withDeadline('Vector store', DEADLINE_MS, () => never);
  ...
  it('does not leave a pending timer holding the event loop open', async () => {
    vi.useFakeTimers();
    await withDeadline('Embedding provider', DEADLINE_MS, () => Promise.resolve(1));
