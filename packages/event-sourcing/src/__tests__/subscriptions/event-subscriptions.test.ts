...
describe('EventSubscriptions', () => {
  ...
  function createStoredEvent(type: ResourceEvent['type'], resId?: string): StoredEvent {
    return {
      event: {
        id: 'event-1',
        type,
  ...
  describe('Resource Subscriptions', () => {
    it('should subscribe to resource events', () => {
      ...
      const sub = subscriptions.subscribe(resourceId('doc1'), callback);
    ...
    it('should notify resource subscribers', async () => {
      const callback = vi.fn();
      subscriptions.subscribe(resourceId('doc1'), callback);

      const event = createStoredEvent('annotation.added', 'doc1');
      await subscriptions.notifySubscribers(resourceId('doc1'), event);
    ...
    it('should support multiple subscribers for same resource', async () => {
      ...
      subscriptions.subscribe(resourceId('doc1'), callback1);
      subscriptions.subscribe(resourceId('doc1'), callback2);
      subscriptions.subscribe(resourceId('doc1'), callback3);
    ...
    it('should not notify subscribers of other resources', async () => {
      ...
      subscriptions.subscribe(resourceId('doc1'), callback1);
      subscriptions.subscribe(resourceId('doc2'), callback2);
    ...
    it('should clean up empty subscription sets', () => {
      const sub = subscriptions.subscribe(resourceId('doc1'), vi.fn());

      expect(subscriptions.getSubscriptionCount(resourceId('doc1'))).toBe(1);

      sub.unsubscribe();

      expect(subscriptions.getSubscriptionCount(resourceId('doc1'))).toBe(0);
    });

    it('should get subscription count for resource', () => {
      subscriptions.subscribe(resourceId('doc1'), vi.fn());
      subscriptions.subscribe(resourceId('doc1'), vi.fn());
      subscriptions.subscribe(resourceId('doc2'), vi.fn());

      expect(subscriptions.getSubscriptionCount(resourceId('doc1'))).toBe(2);
      expect(subscriptions.getSubscriptionCount(resourceId('doc2'))).toBe(1);
      expect(subscriptions.getSubscriptionCount(resourceId('doc-nonexistent'))).toBe(0);
    });
  ...
  describe('Global Subscriptions', () => {
    ...
    it('should not affect resource subscriptions', async () => {
      ...
      subscriptions.subscribe(resourceId('doc1'), docCallback);
      ...
      await subscriptions.notifySubscribers(resourceId('doc1'), docEvent);
  ...
  describe('Fire-and-Forget Pattern', () => {
    it('should not block on slow subscribers', async () => {
      ...
      subscriptions.subscribe(resourceId('doc1'), fastCallback);
      subscriptions.subscribe(resourceId('doc1'), slowCallback);
    ...
    it('should handle errors in async callbacks without affecting others', async () => {
      ...
      subscriptions.subscribe(resourceId('doc1'), goodCallback);
      subscriptions.subscribe(resourceId('doc1'), badCallback);
      subscriptions.subscribe(resourceId('doc1'), anotherGoodCallback);
    ...
    it('should support async callbacks', async () => {
      ...
      subscriptions.subscribe(resourceId('doc1'), asyncCallback);
  ...
  describe('Edge Cases', () => {
    it('should handle notification with no subscribers', async () => {
      ...
      await expect(subscriptions.notifySubscribers(resourceId('doc-nonexistent'), event)).resolves.toBeUndefined();
    });
    ...
    it('should allow re-subscribing after unsubscribing', async () => {
      ...
      const sub1 = subscriptions.subscribe(resourceId('doc1'), callback);
      sub1.unsubscribe();

      const sub2 = subscriptions.subscribe(resourceId('doc1'), callback);
