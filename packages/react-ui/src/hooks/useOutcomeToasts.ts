 ...
 *
 * This is deliberately the whole seam: these six channels need only the
 ...
 *
 * `job:complete`/`job:fail` are filtered to `resourceId`; the mark/bind
 * failure channels carry no resource id and are session-wide (pre-existing
 * behavior — with N mounted viewers each one toasts).
 */
