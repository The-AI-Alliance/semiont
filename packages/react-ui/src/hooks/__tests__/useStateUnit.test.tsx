 ...
 * `useStateUnit`. The hook deliberately keeps a single state-unit instance for
...
describe('useStateUnit', () => {
  it('calls the factory exactly once on initial mount', () => {
    const factory = vi.fn(() => makeStateUnit('vm-1'));
    renderHook(() => useStateUnit(factory));
    expect(factory).toHaveBeenCalledTimes(1);
  });

  it('returns the same state-unit instance across re-renders', () => {
    const factory = vi.fn(() => makeStateUnit('vm-1'));
  ...
  it('does NOT re-run the factory when a closed-over value changes — latent-bug guard', () => {
    ...
    // Factory still not re-invoked; state unit still bound to res-A.
    expect(factory).toHaveBeenCalledTimes(1);
    expect(result.current.id).toBe('res-A');
  });
  ...
  it('a key-bump on a wrapping component forces a fresh state unit — the documented fix for param-dependent factories', () => {
