...
export function CollapsibleResourceNavigation({
  ...
  const handleKeyboardReorder = useCallback((resourceId: string, direction: 'up' | 'down') => {
    ...
    // Emit event
    session?.client.emit('browse:resource-reorder', { oldIndex: currentIndex, newIndex });
  ...
  const handleResourceClose = (resourceId: string, e: React.MouseEvent) => {
    ...
    // Emit event
    session?.client.emit('browse:resource-close', { resourceId });
  ...
  const handleDragEnd = (event: DragEndEvent) => {
    ...
    if (over && active.id !== over.id) {
      ...
      if (oldIndex !== -1 && newIndex !== -1) {
        // Emit event
        session?.client.emit('browse:resource-reorder', { oldIndex, newIndex });
